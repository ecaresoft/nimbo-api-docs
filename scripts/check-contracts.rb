...
spec = YAML.safe_load_file(File.join(root, 'openapi/nimbo_public.yml'))
...
inventory.fetch('files').each do |file, expected|
  doc = YAML.safe_load_file(File.join(root, 'openapi', file))
...
puts "Contracts checked: #{actual.size} public preview operations; #{inventory.fetch('files').values.sum { |v| v.fetch('operations') }} legacy operations"
